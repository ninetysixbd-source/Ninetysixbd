...
export function ProductImageGallery({ images, productName }: ProductImageGalleryProps) {
    ...
    if (images.length === 0) {
        return (
            <div className="aspect-[3/4] relative bg-muted rounded-lg overflow-hidden border flex items-center justify-center">
    ...
    const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        // Only swipe if horizontal movement is greater than vertical (avoid scroll hijacking)
    ...
    return (
        <div className="space-y-4">
            {/* Main Image */}
            <div
                className="aspect-[3/4] relative bg-white rounded-lg overflow-hidden border group cursor-crosshair"
                onMouseMove={(e) => {
            ...
            >
                <ProductImage
                    ...
                    objectFit="contain"
                    preset="detail"
                    className="md:hover:scale-[2] md:hover:origin-[var(--x)_var(--y)]"
                ...
                {images.length > 1 && (
                    <>
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10 pointer-events-none md:hidden">
                            {images.map((_, index) => (
                                <div
                                    key={index}
                                    className={cn(
