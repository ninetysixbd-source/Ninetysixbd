...
export function ProductQuickView({ open, onOpenChange, product }: ProductQuickViewProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl w-[95vw] md:w-full overflow-hidden p-0 max-h-[90vh] overflow-y-auto">
                <DialogTitle className="sr-only">{product.name}</DialogTitle>
                <DialogDescription className="sr-only">Quick view of {product.name}</DialogDescription>
                <div className="grid md:grid-cols-2 gap-0">
                    {/* Image Column */}
                    <div className="bg-gray-100 h-48 md:h-auto flex items-center justify-center relative">
                        {product.images?.[0] ? (
                            <ProductImage
                    ...
                    {/* Details Column */}
                    <div className="p-4 md:p-8 flex flex-col h-full bg-white">
