...
import { serverTrackInitiateCheckout } from "@/app/actions/track-event"
import { fbTrackInitiateCheckout, fbTrackPurchase } from "@/lib/fb-pixel"
...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        if (items.length > 0) {
            ...
            // Client-side pixel (visible in Pixel Helper)
            fbTrackInitiateCheckout(total, numItems)
            // Server-side Conversions API
            serverTrackInitiateCheckout(total, numItems, session?.user?.email || undefined).catch(() => { })
        }
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                fbTrackPurchase(
                    finalTotal,
                    items.map(item => ({ id: item.productId, name: item.name, quantity: item.quantity, price: item.price }))
                )
                clearCart()
                toast.success("Order placed successfully!")
                router.push(`/order-confirmation/${result.orderNumber}`) // Redirect to order confirmation
            } else {
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            {(paymentMethod === "bank" || paymentMethod === "mobile") && (
                                <div className="p-4 bg-orange-50 border border-orange-200 rounded-md flex items-start gap-2 text-sm text-orange-700 animate-in fade-in">
                                    <AlertCircle className="h-5 w-5 text-orange-500 shrink-0 mt-0.5" />
                                    <p>Currently this service is not available. Please select <strong>Cash on Delivery</strong> to place your order.</p>
                                </div>
                            )}
                        </CardContent>
                        <CardFooter>
                            <Button
                                ...
                                disabled={isSubmitting || paymentMethod === "bank" || paymentMethod === "mobile"}
                            >
                                {isSubmitting ? "Processing..." : paymentMethod === "bank" || paymentMethod === "mobile" ? "Service Not Available" : "Confirm Order"}
                            </Button>
                        </CardFooter>
                    </Card>
                </div>
